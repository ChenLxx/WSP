...
#DATA
X_raw=np.zeros((86400,9))

dat1 = pd.read_csv('/home/liuchen/wind_data/K117+378_2017-09-30.txt',header=None)
dat2 = pd.read_csv('/home/liuchen/wind_data/K127+995_2017-09-30.txt',header=None)
dat3 = pd.read_csv('/home/liuchen/wind_data/K135+913_2017-09-30.txt',header=None)
dat4 = pd.read_csv('/home/liuchen/wind_data/K145+688_2017-09-30.txt',header=None)
dat5 = pd.read_csv('/home/liuchen/wind_data/K154+192_2017-09-30.txt',header=None)
dat6 = pd.read_csv('/home/liuchen/wind_data/K164+845_2017-09-30.txt',header=None)
dat7 = pd.read_csv('/home/liuchen/wind_data/K174+468_2017-09-30.txt',header=None)
dat8 = pd.read_csv('/home/liuchen/wind_data/K186+682_2017-09-30.txt',header=None)
dat9 = pd.read_csv('/home/liuchen/wind_data/K196+146_2017-09-30.txt',header=None)
X_raw[:,0]=dat1.iloc[:,1].values
X_raw[:,1]=dat2.iloc[:,1].values
X_raw[:,2]=dat3.iloc[:,1].values
X_raw[:,3]=dat4.iloc[:,1].values
X_raw[:,4]=dat6.iloc[:,1].values
X_raw[:,5]=dat7.iloc[:,1].values
X_raw[:,6]=dat8.iloc[:,1].values
X_raw[:,7]=dat9.iloc[:,1].values
X_raw[:,8]=dat5.iloc[:,1].values
...
X1= X_select[0:1020,0:6]
#targt series
Y1= X_select[0:1020,6]
ROW=X1.shape[0]
COLUMN=X1.shape[1]
'''
...
plt.figure()
plt.plot(range(1, 1+len(Y1[500:700])), Y1[500:700], label = "True")
plt.legend(loc = 'upper left')
...
class rnn_mil(nn.Module):
    def __init__(self,in_dim,T,encoder_hidden_size,decoder_hidden_size,fc_hidden_size,out_dim):
        ...
        self.encoder_attn_linear = nn.Linear(in_features = 2 * self.encoder_hidden_size + T - 1, out_features = 1)
    ...
    def forward(self,X,Y):
        input_encoded = Variable(X.data.new(X.size(0), self.T - 1, self.encoder_hidden_size).zero_())
        #input_encoded= Variable(X.new_zeros(X.size(0), self.T - 1, self.encoder_hidden_size))
        #encoder
        ...
        for t in range(self.T - 1):
            ...
            x = self.encoder_attn_linear(x.view(-1, self.encoder_hidden_size * 2 + self.T - 1)) # (batch_size * input_size) * 1
        ...
        input_final=Variable(torch.randn(BATCH_SIZE,T,decoder_hidden_size)).cuda()
...
in_dim = len(X1[0])
out_dim = 1
encoder_hidden_size = 128  # rnn hidden size
...
model = rnn_mil(in_dim,T,encoder_hidden_size,decoder_hidden_size,fc_hidden_size,out_dim).cuda()
...
optimizer=torch.optim.Adam([{ 'params':weight_p,'weight_decay':0.003},
...
y_test_pred = predict(on_train = False)
y_pred = np.concatenate((y_train_pred, y_test_pred))
file = os.getcwd() + 'pro2.0.csv'
data = pd.DataFrame(y_pred)
#data.to_csv('pro2.0.csv', index=False,header=False) 
data.to_csv('pro2.0.csv', mode='a+',index=False,header=False) 
dat0 = pd.read_csv('pro2.0.csv',header=None)
print(dat0.shape)
...
X_test= X_select[1000:1420,0:6]
#targt series
Y_test= X_select[1000:1420,6]
...
while i < len(y_test_pred):
        ...
        for j in range(len(batch_idx)):
            ...
            #print(input_final)
            #print(mil(input_final).cpu().data.numpy()[:, 0])
        y_test_pred[i:(i + BATCH_SIZE)] = model(Variable(torch.from_numpy(X).type(torch.FloatTensor).cuda()),
                                              Variable(torch.from_numpy(y_history).type(torch.FloatTensor).cuda())).cpu().data.numpy()[:, 0]
            #print('here1')
            #y_pred[i:(i + BATCH_SIZE)] = mil(Variable(torch.from_numpy(input_final).type(torch.FloatTensor).cuda()))
            #y_pred.append(mil(input_final))
